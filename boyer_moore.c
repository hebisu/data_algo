...
void print_comparison_process(const char text[], const char pattern[], const int ptr_text, const int ptr_pattern){
...
int boyermoore_match(const char text[], const char pattern[], const int print_process)
{
	...
	while (ptr_text < text_len) { // Continue to the end of text
		...
		if(print_process) print_comparison_process(text, pattern, ptr_text, ptr_pattern);
